...
class HyOptimize:
    def __init__(self, estimator=None, params=None, cv=5, grid=True, stratified=False, regression=False):
        if stratified:
    ...
    def BayesianOpt(self, X, y, estimator, params, cv, scoring):
        ms.cross_val_score(estimator, X, y, cv=cv, scoring=scoring)
        pass

    def RandomOpt(self, X, y, estimator, params, cv, scoring):
        pass

    def CoordinateAscent(self, X, y, estimator, params, cv, scoring):
        name = estimator.__class__.__name__  
        res, i, means, stds= {}, 0, [], []
        temp = {key:value for key, value in estimator.get_params().items() if key in params}
        while i<=20 and res != temp:
            res,temp = temp, { }
            stepmean, stepstd = [], []
            for each in params:
                grid = ms.GridSearchCV(estimator,param_grid={each:params[each]},scoring=scoring, cv=cv, iid=True,
                        n_jobs=-1)
                grid.fit(X, y)
    ...
    def GridSearch(self, X, y, estimator, params, cv, scoring):
        name = estimator.__class__.__name__  
        grid = ms.GridSearchCV(estimator, param_grid=params,scoring=scoring, cv=cv, iid=True,
                               n_jobs=-1)
        grid.fit(X, y)
        self.res[name+'_mean'] = grid.cv_results_['mean_test_score'] # 保存搜索过程中的结果
        self.res[name+'_std'] = grid.cv_results_['std_test_score']
        self.estimator = clone(grid.best_estimator_)
        return grid.best_score_, grid.best_params_

    def hyparameter(self, X, y, estimator=None, params=None, cv=None, scoring=None, grid=None):
    ...
    def plot_cv_curve(self, results, x=None):
        ...
        for color, (key,result) in zip(colors, results.items()):
            ...
            ax.fill_between(x, mean+std, mean-std, alpha=0.1, 
                            color=color[1])
        return ax
    def plot_learning_curve(self, estimator1, X, y, cv=5,
                        train_sizes=np.linspace(.1, 1.0, 5)):
        name = estimator1.__class__.__name__
        train_sizes, trains, tests = ms.learning_curve(
        estimator1, X, y, cv=cv, n_jobs=-1, train_sizes=train_sizes)
        ax = self.plot_cv_curve({'train':trains,'test':tests},x=train_sizes)
