...
class ReadData:
    ...
    def plot_bc(self, y):  # 可视化box-cox
        ...
        print('After Transform Kurt:{:.2f}, Skew:{:.2f}'.format(
            data.kurt(), data.skew()))
    ...
    def reduce_mem(self, df, verbose=False):
        ...
        result = df.copy()
        result.loc[:, columns[intmask]] = result.loc[:,
                                                     columns[intmask]].astype(
                                                         np.int32)
        result.loc[:,
                   columns[floatmask]] = result.loc[:,
                                                    columns[floatmask]].astype(
                                                        np.float32)
        if verbose:
