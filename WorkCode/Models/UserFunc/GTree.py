...
class XGB(GradientTree):
    def __init__(self, imbalance=False):
        ...
        self.estimator = xgb.XGBClassifier().set_params(**self.params)
        self.imbalance = imbalance
        self.scoring = 'roc_auc'
...
class LGBM(GradientTree):
    def __init__(self, imbalance=False):
        ...
        self.estimator = lgb.LGBMClassifier().set_params(**self.params)
        self.imbalance = imbalance
        self.scoring = 'roc_auc'
...
class GBDT(GradientTree):
    def __init__(self, imbalance=False):
        ...
        self.param_grid = {
            ...
            'max_leaf_nodes': np.arange(5, 31, 5),
        }
        self.estimator = GradientBoostingClassifier().set_params(**self.params)
        self.imbalance = imbalance
        self.scoring = 'roc_auc'
    ...
    def get_estimators(self, X, y, params=None):
        ...
        for tid, vid in cv.split(X, y):
            ...
            result.append(
                list(
                    map(lambda y, ypred: roc_auc_score(y, ypred[:, 1]),
                        repeat(validy),
                        estimator.staged_predict_proba(validx))))
