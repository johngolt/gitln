...
class RNNModel(object):
    def __init__(self, look_back=1, epochs_purchase=20, epochs_redeem=40, 
                           batch_size=1, verbose=2, patience=10, store_result=False):
        ...
        self.purchase = pandas.read_csv('./total_amt.csv', usecols=[1], 
                                        engine='python') 
        self.redeem = pandas.read_csv('./total_amt.csv', usecols=[2], 
                                        engine='python')
    ...
    def access_data(self, data_frame):
        ...
        # or tanh activation functions are used. It can be a good practice to rescale the data to the range of
        #  0-to-1, also called normalizing.
    ...
    def rnn_model(self, train_x, train_y, epochs):
        ...
        history = model.fit(train_x, train_y, epochs=epochs, batch_size=self.batch_size, 
    ...
    def run(self):
        purchase_train_x, purchase_train_y, purchase_test, purchase_scaler = self.access_data(
            self.purchase)
        ...
        if self.store_result is True:
            test_user.to_csv('submit_lstm.csv', encoding='utf-8', index=None)
...
if __name__ == '__main__':
    initiation = RNNModel(look_back=40, epochs_purchase=150, epochs_redeem=230, 
    batch_size=16, verbose=2, patience=50, store_result=False)
    initiation.run()