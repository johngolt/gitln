...
weight2 = variable_with_weight_loss(shape=[5, 5, 64, 64], stddev = 5e-2,
                                    w1 = 0.)
kernel2 = tf.nn.conv2d(norm1, weight2, [1, 1, 1, 1],
                       padding = 'SAME')
bias2 = tf.Variable(tf.constant(0.1, shape = [64]))
conv2 = tf.nn.relu(tf.nn.bias_add(kernel2, bias2))
norm2 = tf.nn.lrn(conv2, 4, bias = 1., alpha = 0.001/9.,
                  beta = 0.75)
...
def loss(logits, labels):
    ...
    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits = logits, labels = labels, name = 'cross'
    )
    cross_entropy_mean = tf.reduce_mean(cross_entropy, name = 'cross_mean')
